...
public class CollectionInfoService {
    ...
    @Resource(name="CollectionInfoWebClient")
    private final WebClient webClient;
    private final PTPWebClientConfig ptpWebClientConfig;

    public CollectionInfoResponse getCollectionInfoDetails(String ban, RequestHeaders requestHeaders){
        return webClient.get().uri(uriBuilder -> uriBuilder.path(ptpWebClientConfig.getCollectionInfoUri()).build(ban))
                .headers(httpHeaders -> {
